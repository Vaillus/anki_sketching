...
A **tag** is a free-form string attached to a card. Tags live entirely in this app — they are *not* synced from or to Anki. They're used for one main purpose: **filtering the Learn page** to focus a review session on a subset of cards (e.g. only "perspective" cards, excluding "anatomy").
...
Below the card content, as a row of small pills (`.card-tag`). Rendered by `buildTagsHTML()` in `globals.js` (Build) and `buildLearnTagsHtml()` in `learn/main.js` (Learn). Same visual style across both pages.

### 2. Selection toolbar (Build page)

Appears at the bottom of the Build page when ≥1 card is selected (`#selection-tags-list`):
...
Filter predicate (`learn/main.js::getFilteredCards`):
...
Filtering is **client-side** — the full queue is loaded once and re-filtered in JS. The `learn-count` element shows `filtered/total` when a filter is active.

## Cache

`allTagsCache` (in `globals.js`) is the frontend's cached list of all known tags, refreshed by `loadAllTags()` (called on Build page load and after every add/remove action). It's the source for both autocompletes (selection toolbar and local-card modal).

The Learn page has its own load: `loadAllTagsLearn()` on DOMContentLoaded. The two are not kept in sync — if the Learn page is open and tags change on the Build page, the Learn filter won't update until reload.

## What this spec does not cover

- The visual styling of pills → `frontend/static/css/app.css` and `learn.css`.
...
- **No persistence of filter state.** Refreshing the Learn page resets all filter pills to neutral.
...
- **Cache invalidation across pages.** The Build and Learn pages each have their own tag cache, refreshed only on actions performed on that page.
